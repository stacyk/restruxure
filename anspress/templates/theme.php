...
function ap_list_filters( $current_url = '' ) {
	...
	foreach ( (array) $filters as $key => $filter ) {
		...
		echo '<a class="ap-dropdown-toggle ap-filter-toggle" href="#" ap-filter ap-query="' . esc_js( $args ) . '">
			<span class="ap-filter-toggle-label">' . esc_attr( $filter['title'] ) . ': </span>' . $active . '</a>'; // xss okay.
		echo '</div>';
	}
